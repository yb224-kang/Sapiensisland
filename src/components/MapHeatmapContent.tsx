...
export default function MapHeatmapContent({ selectedExpert, dateRange }: MapHeatmapContentProps) {
  ...
  const treemapData = regionStats.map(([region, count]) => {
    ...
    let color;
    if (diffFromAvg > 50) {
      color = '#16a34a'; // 강한 초록 (평균 대비 +50% 이상)
    } else if (diffFromAvg > 20) {
      color = '#22c55e'; // 초록 (평균 대비 +20~50%)
    } else if (diffFromAvg > 0) {
      color = '#4ade80'; // 연한 초록 (평균 대비 +0~20%)
    } else if (diffFromAvg > -20) {
      color = '#f87171'; // 연한 빨강 (평균 대비 -0~-20%)
    } else if (diffFromAvg > -50) {
      color = '#ef4444'; // 빨강 (평균 대비 -20~-50%)
    } else {
      color = '#dc2626'; // 강한 빨강 (평균 대비 -50% 이하)
    }
